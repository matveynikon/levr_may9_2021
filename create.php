...
arr2 = []
var daylist = getDaysArray(new Date('2021-04-01'),new Date('2021-04-7'));
...
async function start() {
  ...
  console.log(someVar);//          https://twitter.com/search?q=(from%3' + someVar + ')%20until%3A2021-04-3%20since%3A2021-03-28%20-filter%3Areplies&src=typed_query&f=live
  ...
  for(f = 0; f < 200; f++){
      ...
      page.evaluate(_ => {        
          window.scrollBy(200, window.innerHeight);
      });
  } 
  ...
  for(i = 0; i < dhyper.length; i++){
    ...
    fs.writeFile('ids.txt', dhyper[i][1], (err) => {
        if (err)
          ...
          console.log(fs.readFileSync('ids20.txt', 'utf8'));
        }
    });
