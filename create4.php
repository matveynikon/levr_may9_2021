...
for($k = 0; $k < 150000; $k++){
    sleep(2);
    $file = fopen("ids.txt", "r") or die("Unable to open file!");
...
for($k2 = 0; $k2 < 20; $k2++){
    sleep(1);
    $file2 = fopen("ids.txt", "r") or die("Unable to open file!");
    //$text = fread($myfile,filesize("ids.txt"));
    $text2 = fgets($file2);
    if( $text === $text2){
    }
...
for($k3 = 0; $k3 < 20; $k3++){
    sleep(1);
    $file3 = fopen("ids.txt", "r") or die("Unable to open file!");
    //$text = fread($myfile,filesize("ids.txt"));
    $text3 = fgets($file3);
    if( $text2 === $text3){
        if($k3 > 5){
...
for($k4 = 0; $k4 < 20; $k4++){
    sleep(1);
    $file4 = fopen("ids.txt", "r") or die("Unable to open file!");
    //$text = fread($myfile,filesize("ids.txt"));
    $text4 = fgets($file4);
    if( $text3 === $text4){
        if($k4 > 5){
...
for($k5 = 0; $k5 < 20; $k5++){
    sleep(1);
    $file5 = fopen("ids.txt", "r") or die("Unable to open file!");
    //$text = fread($myfile,filesize("ids.txt"));
    $text5 = fgets($file5);
    if( $text4 === $text5){
        if($k5 > 5){
...
<div class="flexbox-centering">
  <div><?=$handle?>'s top 5 most retweeted tweets from <?=$s2?> to <?=$e2?></div>
</div>
<div id="demo"></div>
<div id="demo2"></div>
<div id="demo3"></div>
...
const repeatedGreetings = async () => {
  ...
  rank = [];
  var text = "";
  text += "<blockquote class='twitter-tweet'><a href='https://twitter.com<?=$fullurl?>?ref_src=twsrc%5Etfw'></a></blockquote>";
  document.getElementById("demo").innerHTML = text;
  var text2 = "";
  text2 += "<blockquote class='twitter-tweet'><a href='https://twitter.com<?=$fullurl2?>?ref_src=twsrc%5Etfw'></a></blockquote>";
  document.getElementById("demo2").innerHTML = text2;
  try{
    var text3 = "";
    text3 += "<blockquote class='twitter-tweet'><a href='https://twitter.com<?=$fullurl3?>?ref_src=twsrc%5Etfw'></a></blockquote>";
...
<div class="info2">&#9432; Twitter can't show an embedded tweet's number of retweets, thus these tweets are rendered in order of most retweets, but can't display these tweets' actual number retweets.</div>
<?php
unlink("ids.txt");
echo("unlinked!!!!!!!!!!");
?>
